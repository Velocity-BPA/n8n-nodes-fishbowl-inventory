...
export async function get(
  ...
  let endpoint: string;
  if (byName) {
    endpoint = `/customer?name=${encodeURIComponent(customerId)}`;
  } else {
...
export async function getAll(
  ...
  if (returnAll) {
    ...
    const response = await fishbowlApiRequest.call(this, 'GET', '/customer', {}, query);
    const items = response.results || response.customers || [];
...
export async function create(
  ...
  // Handle default salesman
  if (additionalFields.defaultSalesmanId) {
    body.defaultSalesman = { id: additionalFields.defaultSalesmanId };
    delete body.defaultSalesmanId;
  }
  ...
  const response = await fishbowlApiRequest.call(this, 'POST', '/customer', body);
  return simplifyOutputData([response]);
}
...
export async function update(
  ...
  // Handle references
  if (updateFields.defaultSalesmanId) {
    body.defaultSalesman = { id: updateFields.defaultSalesmanId };
    delete body.defaultSalesmanId;
  }
  ...
  const response = await fishbowlApiRequest.call(this, 'PUT', `/customer/${customerId}`, body);
  return simplifyOutputData([response]);
}
...
export async function deleteCustomer(
  ...
  await fishbowlApiRequest.call(this, 'DELETE', `/customer/${customerId}`);
  return simplifyOutputData([{ success: true, customerId }]);
}
...
export async function getAddresses(
  ...
  const response = await fishbowlApiRequest.call(this, 'GET', `/customer/${customerId}/addresses`);
...
export async function getContacts(
  ...
  const response = await fishbowlApiRequest.call(this, 'GET', `/customer/${customerId}/contacts`);
...
export async function getOrders(
  ...
  const orders = response.results || response.salesOrders || [];
